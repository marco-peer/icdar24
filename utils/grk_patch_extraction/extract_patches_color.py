...
def load_keypoints(imgp, keypoint_dir):
    kp_file = os.path.join(keypoint_dir[0], Path(imgp).stem + '_keypoints.npy')
    kps = np.load(kp_file)
    cv2_keypoints = []
    for idx in range(kps.shape[0]):
        cv2_keypoints.append(cv2.KeyPoint(kps[idx, 0], kps[idx, 1], 1.0))
...
def calc_features(input_img, arguments):
    ...
    kp = load_keypoints(input_img, arguments.keypoint_dir) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--keypoint_dir', metavar='keypoint_dir', dest='keypoint_dir', type=str, nargs=1,
                        help='input directory', required=True)
    ...
    parser.add_argument('--sigma', type=float, help='blur factor for SIFT',
                        default=1.6)
    parser.add_argument('--pca_dim', type=int, help='pca_dimension',
                        default=32)
    parser.add_argument('--ratio', type=float, help='ratio for lowe test',
                        default=0.9)
    ...
    pca = PCA(args.pca_dim, whiten=True)
