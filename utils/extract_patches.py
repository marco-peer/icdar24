...
def calc_features(input_img, arguments):
    ...
    for p in kp:
        ...
        roi = get_image_patch(img_bin, point[0], point[1], arguments)
...
def extract_patches(filename, tup, args):
    ...
    for p, c in zip(points, clusters):
        roi = get_image_patch(img_bin, p[0], p[1], args)
        if roi is None:
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('--patches_per_page', metavar='patches_per_page', dest='patches_per_page', type=int, nargs='?',
                        help='maximal numb0er of patches per page (-1 for no limit)',
                        default=-1)
    ...
    parser.add_argument('--centered', type=bool, help='filter patches whose keypoints are not located on handwriting, only for binarized datasets',
                    default=False)
    parser.add_argument('--pca_dimension', type=int, default=32, help='pca dimension of sift descriptors for clustering')
    parser.add_argument('--ratio', type=float, default=0.9, help='pca dimension of sift descriptors for clustering')
    ...
    pca = PCA(args.pca_dimension, whiten=True)
    ...
    kmeans = sklearn.cluster.MiniBatchKMeans(n_clusters=args.number_of_clusters, compute_labels=False,
                                             batch_size=10000 if args.number_of_clusters <= 1000 else 50000, init='k-means++')
